...
#rados -p default.rgw.buckets.data ls > rgw_rados_object_names.txt
#rados -p $data_pool ls > ${implementation}_rados_object_names.txt
...
for size in 8 16 32 64 128 256 512 1024 2048 4096; do
	...
	for osd_id in "osd.0" "osd.1" "osd.2"; do
		cat ${osd_id}.txt | grep "$((size * 1024))" | awk -F, -v osd_id="${osd_id}" '{printf("/%dkb/%s\n", $1/1024, $3);}' > ${implementation}_${size}kb_obj_in_${osd_id}.txt
		#cat ${osd_id}.txt | grep "$((size * 1024))" | awk -F, -v osd_id="${osd_id}" '{printf("%dkb,%s,%d,%s\n", $1/1024, $3, $1, osd_id);}' > ${implementation}_${size}kb_obj_in_${osd_id}.txt
		#cat ${osd_id}.txt | grep "$((size * 1024))" | awk -F, '{printf("%dkb,%s,%d\n", $1/1024, $2, $1);}' > ${implementation}_${size}kb_obj_in_${osd_id}.txt
	...
	#cat "${implementation}_${size}kb_obj_in_osd.0.txt" "${implementation}_${size}kb_obj_in_osd.1.txt" "${implementation}_${size}kb_obj_in_osd.2.txt" "${implementation}_${size}kb_obj_in_osd.3.txt" | shuf | shuf > "${implementation}_${size}kb_obj_balanced.txt"
