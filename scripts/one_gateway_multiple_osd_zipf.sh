...
if [ $# -ne 6 ]; then
	echo "$0 [webserver name] [gateway hostname] [link speed] [experiment name] [n_obj_to_select] [alpha]"
	exit
fi
...
#	mkdir -p "${base_dir}/${link_speed}/${case}_zipf${exponent}"
#	python3 select_zipf.py "${inputfile_dir}/${technology}_${size}kb_obj_balanced.txt" "$n_obj_to_select" "$exponent" > "${base_dir}/${link_speed}/${case}_zipf${exponent}/obj_list_${size}KiB.txt"
#done
...
for size in 4096 2048 1024 512 256 128 64 32 16 8; do
	for forwarding_method in "hybrid" "eBPF" "HW-TC"; do
		...
		input_file="${base_dir}/${link_speed}/${case}_zipf${exponent}/obj_list_${size}KiB.txt"
		output_dir="${base_dir}/${link_speed}/${case}_zipf${exponent}/${forwarding_method}/${size}kib"
		mkdir -p "$output_dir"
	...
	#mv /root/xo-server/build/n*.txt "${base_dir}/${link_speed}/${case}_zipf${exponent}/${forwarding_method}/"
done
